...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Bean
	public UserDetailsService customUserServic() {
		return new CustomAuthMemberService();
	}

	@Override
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(customUserServic()).passwordEncoder(bcPasswordEncoder());
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// 커스텀 로그인 페이지 구성
		http.formLogin().usernameParameter("email") // id에 해당하는 거(email을 id로 사용할거임)
				.passwordParameter("pwd") // 비밀번호에 해당하는 거(pwd)
				.loginPage("/member/login") // comtroller에 주소요청 맵핑도 같이 꼭 적어줘야 함.
				.successHandler(authSuccessHandler())// 로그인 성공 시 authSuccessHandler메서드를 타라
				.failureHandler(authFailurHandler());// 로그인 실패 시 authFailurHandler메서드를 타라

		// 로그아웃 페이지
		http.logout().logoutUrl("/member/logout") // 반드시 method="post"(nav.jsp에 있음)
